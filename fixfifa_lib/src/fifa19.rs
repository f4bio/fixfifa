...
pub fn enable_event_hook(process: &Process, on_event: LPARAM) -> Result<()> {
  let module_info: ModuleInfo = process.get_module_info("fifa19.exe").unwrap();
  let pattern_addr: LPARAM = process
    .find_pattern(module_info.start, module_info.end, &pattern::EVENT)
    .unwrap();
  let inject_addr: LPARAM = process
    .find_space(pattern_addr, module_info.end, EVENT_HOOK_T)
    .unwrap();
  let hook_addr: LPARAM = pattern_addr + EVENT_HOOK_OFFSET_T as LPARAM;
  let return_addr: LPARAM = process
    .write_rel_target(hook_addr, &[0xE9], inject_addr)
    .unwrap();
  ...
  addr = process
    .write_rel_target(addr, &[0xE9], return_addr)
    .unwrap();
...
pub fn disable_quit_on_loose_focus(process: &Process) -> Result<()> {
  ...
  if pattern_addr != 0 {
    let target_addr = process
      .read_rel_target(pattern_addr + offset_je, 6)
      .unwrap();
    let addr_nop = process
      .write_rel_target(pattern_addr + offset_je, &[0xE9], target_addr)
      .unwrap();
...
fn console(enabled: bool) -> Result<()> {
  ...
  let success =
    unsafe { ShowWindow(window, if enabled { SW_SHOW } else { SW_HIDE }) };
  if success == FALSE {
