...
use winapi::shared::minwindef::{
  BYTE, DWORD, FALSE, HIBYTE, HIWORD, HMODULE, LOBYTE, LOWORD, LPARAM, LPCVOID,
  LPVOID, MAKELONG, MAKEWORD, WORD, WPARAM,
};
...
fn to_lparam(bytes: &[BYTE]) -> Result<LPARAM> {
  ...
  return Ok(
    MAKELONG(MAKEWORD(bytes[0], bytes[1]), MAKEWORD(bytes[2], bytes[3]))
      as LPARAM,
  );
}
...
impl Process {
  ...
  pub fn read_addr(&self, address: LPARAM) -> Result<LPARAM> {
    let bytes: Vec<BYTE> =
      self.read_memory(address, RELATIVE_ADDRESS_T).unwrap();
    if bytes.len() != RELATIVE_ADDRESS_T {
      return Err(ProcessError::new("Invalid address size."));
    }
  ...
  pub fn write_addr(
    ...
    if size == DWORD_T {
      return self
        .write_memory(address, &to_dword_bytes(addr as WPARAM).unwrap());
    } else if size == QWORD_T {
      return self
        .write_memory(address, &to_qword_bytes(addr as WPARAM).unwrap());
    }
    return Err(ProcessError::new("Invalid address size."));
  ...
  pub fn write_rel_target(
    ...
    // Relative target address is always 4 bytes.
    let rel_addr: LPARAM =
      target - address - ((op.len() + RELATIVE_ADDRESS_T) as LPARAM);
  ...
  pub fn get_module(&self, name: &str) -> Result<HMODULE> {
    ...
    let module_handle: HMODULE =
      unsafe { GetModuleHandleA(c_str.as_ptr() as LPCSTR) };
